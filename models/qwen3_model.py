...
class Qwen3Model(LlmBaseModel):
    r"""
    @class Qwen3Model
    @brief Inherit LlmBaseModel and implement the inference function of Qwen3.
    """
    def __init__(self, 
                 rpp_dir: str,
                 input_size: int,
                 ...
                 print_tokens: int=1):
        r"""
        Class init function, construct the Qwen3Model class and initialize the parameters.
        ...
        """
        super(Qwen3Model, self).__init__(rpp_dir=rpp_dir,
                                          graph_engine=None, 
                                          low_power=low_power,
    ...
    def _apply_chat_prompt(self, prompt: Union[str, List], tools: List=None):
        ...
        fixed_tensor = torch.full((1, right_padding), self.tokenizer.eos_token_id, dtype=torch.int)
    ...
    def rpp_inference(self, **kwargs):
        ...
        if self.low_power:
            self._graph_set_power_mode(0)
        # for decoding
        self._warmup(len(original_input_id) * 128)
    ...
    async def rpp_inference_stream(self, **kwargs):
        r"""
        Inference the prompt with Qwen3 running on RPP for async.
