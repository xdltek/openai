...
class Qwen2Model(LlmBaseModel):
    r"""
    @class Llama3Model
    @brief Inherit LlmBaseModel and implement the inference function of Qwen2.
    """
    def __init__(self, 
                 rpp_dir: str,
                 input_size: int,
                 ...
                 print_tokens: int=1):
        r"""
        Class init function, construct the Qwen2Model class and initialize the parameters.
        ...
        """
        super(Qwen2Model, self).__init__(rpp_dir=rpp_dir,
                                         graph_engine=None, 
                                         low_power=low_power,
    ...
    def _build_cpu_models(self):
        ...
        """
        gather_0_names = {
            1024: 'loop1_gather_0_sim.onnx',
            2048: 'loop1_gather_0_2048_sim.onnx',
            4096: 'loop1_gather_0_4096_sim.onnx',
            7168: 'loop1_gather_0_7168_sim.onnx',
            8192: 'loop1_gather_0_8192_sim.onnx',
        }
        gather_1_names = {
            1024: 'loop1_gather_1_sim.onnx',
            2048: 'loop1_gather_1_2048_sim.onnx',
            4096: 'loop1_gather_1_4096_sim.onnx',
            7168: 'loop1_gather_1_7168_sim.onnx',
            8192: 'loop1_gather_1_8192_sim.onnx',
        }
        self.prefill_rotary_emb0 = {
            '1024': onnxruntime.InferenceSession(os.path.join(self.rpp_dir, 'model_files', gather_0_names[1024])),
            '4096': onnxruntime.InferenceSession(os.path.join(self.rpp_dir, 'model_files', gather_0_names[4096])),
            '7168': onnxruntime.InferenceSession(os.path.join(self.rpp_dir, 'model_files', gather_0_names[7168])),
            '8192': onnxruntime.InferenceSession(os.path.join(self.rpp_dir, 'model_files', gather_0_names[8192]))
        }
        self.prefill_rotary_emb1 = {
            '1024': onnxruntime.InferenceSession(os.path.join(self.rpp_dir, 'model_files', gather_1_names[1024])),
            '4096': onnxruntime.InferenceSession(os.path.join(self.rpp_dir, 'model_files', gather_1_names[4096])),
            '7168': onnxruntime.InferenceSession(os.path.join(self.rpp_dir, 'model_files', gather_1_names[7168])),
            '8192': onnxruntime.InferenceSession(os.path.join(self.rpp_dir, 'model_files', gather_1_names[8192]))
        }
        self.decode_rotary_emb0 = onnxruntime.InferenceSession(os.path.join(self.rpp_dir, 'model_files', 'loop2_gather_0_sim.onnx'))
        self.decode_rotary_emb1 = onnxruntime.InferenceSession(os.path.join(self.rpp_dir, 'model_files', 'loop2_gather_1_sim.onnx'))
        try:
    ...
    def _run_prefill_inference(self, input_ids, original_input_ids, attention_mask, position_ids, ppl_flag=False):
        ...
        if len(original_input_ids) < 1024:
            gather_out_0 = self.prefill_rotary_emb0['1024'].run(['output'], {'position_ids': position_ids})[0]
            gather_out_1 = self.prefill_rotary_emb1['1024'].run(['output'], {'position_ids': position_ids})[0]
        elif len(original_input_ids) < 4096:
            gather_out_0 = self.prefill_rotary_emb0['4096'].run(['output'], {'position_ids': position_ids})[0]
            gather_out_1 = self.prefill_rotary_emb1['4096'].run(['output'], {'position_ids': position_ids})[0]
        elif len(original_input_ids) < 7168:
            gather_out_0 = self.prefill_rotary_emb0['7168'].run(['output'], {'position_ids': position_ids})[0]
            gather_out_1 = self.prefill_rotary_emb1['7168'].run(['output'], {'position_ids': position_ids})[0]
        else:
            gather_out_0 = self.prefill_rotary_emb0['8192'].run(['output'], {'position_ids': position_ids})[0]
            gather_out_1 = self.prefill_rotary_emb1['8192'].run(['output'], {'position_ids': position_ids})[0]
    ...
    def rpp_inference(self, **kwargs):
        r"""
        Inference the prompt with Qwen2 running on RPP.
        ...
        # attention_mask = np.ones((1, 1, 1, self.target_len), dtype=np.float32) * (-1 * math.exp(30))
        ...
        while token.tolist()[0] != self.tokenizer.eos_token_id and not self.stop_infer:
            ...
            self.gather_out_0 = self.decode_rotary_emb0.run(['output'], {'position_ids': position_ids_decode})[0]
            self.gather_out_1 = self.decode_rotary_emb1.run(['output'], {'position_ids': position_ids_decode})[0]
    ...
    async def rpp_inference_stream(self, **kwargs):
        r"""
        Inference the prompt with Qwen2 running on RPP for async.
        ...
        if len(original_input_id) < 1024:
            ...
            gather_out_0 = self.prefill_rotary_emb0['4096'].run(['output'], {'position_ids': position_ids})[0]
            gather_out_1 = self.prefill_rotary_emb1['4096'].run(['output'], {'position_ids': position_ids})[0]
        elif len(original_input_id) < 7168:
            gather_out_0 = self.prefill_rotary_emb0['7168'].run(['output'], {'position_ids': position_ids})[0]
            gather_out_1 = self.prefill_rotary_emb1['7168'].run(['output'], {'position_ids': position_ids})[0]
        else:
            gather_out_0 = self.prefill_rotary_emb0['8192'].run(['output'], {'position_ids': position_ids})[0]
